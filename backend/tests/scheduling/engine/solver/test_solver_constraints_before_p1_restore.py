...
def build_constraint_problem(
    *,
    ...
    monday_p1_period = uuid4()

    # Monday P1 is a real institutional period reserved for Assembly.
    #
    # P2 and P3 remain the ordinary teaching periods used by the
    # generic constraint tests. P1 is appended so existing tests
    # relying on problem.periods[0] and problem.periods[1] remain stable.
    periods = (
        PeriodEntity(
            id=monday_period,
            number=2,
...
target = Path(r"tests\scheduling\engine\solver\test_solver_constraints.py")
backup = target.with_name(
    target.stem + "_before_p1_restore.py"
)
...
print("BACKUP :", backup)
print("REWRITE:", target)
print("P1 RESTORED AS REAL PERIOD + SLOT")
