...
def build_timetable_slots(
    ...
    for day_index, day in enumerate(weekdays):
        # The production calendar has ten teaching periods. Apply the
        # authoritative nine-period Monday only for that configured shape;
        # smaller fixtures remain usable for focused engine tests.
        day_periods = (
            teaching_periods[:9]
            if len(teaching_periods) == 10 and day_index == 0
            else teaching_periods
        )
