...
function teacherCode(entry: TimetableEntryResult): string {
  return (
    text(entry.teacher_code) ||
    text(entry.employee_code) ||
    ""
  );
}
...
function dayName(entry: TimetableEntryResult): string {
  return normalise(entry.day || entry.day_display);
}
...
function isGrade10Entry(entry: TimetableEntryResult): boolean {
  ...
  return (
    ...
    group === "10e" ||
    group === "10w" ||
    group.includes("10 east") ||
    group.includes("10 west")
  );
}
...
function classMatches(
  ...
  if (definition.grade10Master) {
    return isGrade10Entry(entry);
  }
...
function formatLesson(entry: TimetableEntryResult) {
  const code = normalisedSubjectCode(entry);
  const name = subjectName(entry);
  const teacher = teacherCode(entry);
  const elective = electiveLabel(entry);
...
function Timetable() {
  ...
  const loadTimetable = useCallback(async () => {
    ...
    try {
      ...
      const result = await getSchedulingRunResults(
        latestRun.id,
      );
      ...
      console.log("============================================================");
console.log("SMARTTIMETABLE LIVE DATA DIAGNOSTIC");
console.log("============================================================");
console.log("Latest completed run:", latestRun);
console.log("Run result:", result);
console.log("Timetable version:", timetableVersion);
console.log("Backend entries count:", backendEntries.length);
console.log("Backend entries:", backendEntries);

backendEntries.forEach((entry, index) => {
  console.log(`ENTRY ${index + 1}:`, {
    id: entry.id,
    day: entry.day,
    day_display: entry.day_display,
    period_number: entry.period_number,
    instructional_group_name: entry.instructional_group_name,
...
console.log("============================================================");
  ...
  function getSlotEntries(
    ...
    const slot = entriesBySlot.get(
      `${normalise(day)}:${period}`,
    ) || [];
    ...
    <>
      ...
      <main className="tt-page">
        ...
        <footer className="tt-footer">
          <span>
            Subject • Teacher Code • Elective Option
          </span>

          <span>
            ✓ {scheduledEntries} backend entries
          </span>
