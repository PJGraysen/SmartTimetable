...
class SolverModelBuilder:
    ...
    def _add_simultaneous_subject_constraints(
        ...
        *,
        ...
        """
        Subjects belonging to the same Grade 10 teaching-group
        combination must occupy identical day/period slots.

        Each subject remains independently assigned to its own teacher
        and room through the existing assignment variables and clash
        constraints.
        """
        ...
        for variable in variables:
            if variable.lesson_requirement_id in variables_by_requirement:
                variables_by_requirement[
                    variable.lesson_requirement_id
                ].append(variable)
        ...
        for (
            ...
            if len(weekly_counts) != 1:
                raise ValueError(
                    "Simultaneous teaching group has mismatched "
                    "weekly lesson counts for instructional group "
                    f"{instructional_group_id}: "
                    f"subjects={sorted(block)}, "
                    f"weekly_counts={sorted(weekly_counts)}."
                )

            # Build the complete set of candidate day/period slots
            # available to any member of this teaching group.
            slots: set[tuple[object, UUID]] = set()
            ...
            # For every possible slot, every subject in the block must
            # have exactly the same occupancy indicator.
            #
            # Therefore:
            #
            # BIO(slot) == MUSIC(slot) == FRE(slot)
            #
            # and similarly for the other combinations.
            for day, period_id in slots:

                occupancy_expressions: list[cp_model.LinearExpr] = []
                ...
                for requirement_id in requirement_ids:
                    ...
                    occupancy_expressions.append(
                        sum(slot_variables)
                    )

                first_expression = occupancy_expressions[0]

                for other_expression in occupancy_expressions[1:]:
                    model.add(
                        first_expression == other_expression
                    )
